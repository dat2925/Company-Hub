...
export function ResourceForm({ config, item, fixedValues, onSubmit, onCancel, busy }: { config: ResourceConfig; item?: Item; fixedValues?: Record<string, string>; onSubmit: (data: Record<string, string>) => void; onCancel: () => void; busy: boolean }) {
  ...
  const departmentId = watch('departmentId') || fixedValues?.departmentId;
  ...
  return (
    <form onSubmit={handleSubmit(values => onSubmit(Object.fromEntries(Object.entries(values).filter(([, v]) => v !== ''))))}>
        ...
        {config.fields.map(f => {
          ...
          return (
            <div key={f.name} className={f.type === 'textarea' ? 'md:col-span-2' : ''}>
              ...
              {f.type === 'textarea' ? (
                <textarea className="input min-h-28 resize-y bg-white/90" {...registration} />
              ) : f.type === 'select' ? (
                <select
                  className="input cursor-pointer bg-white/90 font-semibold"
                  {...registration}
                  onChange={event => {
                ...
                <input type={f.type ?? 'text'} className="input bg-white/90 font-semibold" {...registration} />
              )}
      ...
      <div className="flex justify-end gap-3 mt-8 pt-5 border-t border-slate-100">
        <button type="button" className="btn btn-secondary px-6 font-bold" onClick={onCancel}>
          {t('common.actions.cancel')}
        </button>
        <button disabled={busy} className="btn btn-primary px-7 font-bold shadow-lg shadow-indigo-500/25">
