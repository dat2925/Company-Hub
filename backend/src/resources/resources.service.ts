...
import { DepartmentResourcePaginationDto, IssuePaginationDto, PaginationDto, pageMeta } from '../common/pagination.dto';
import { AuthUser } from '../common/types';
import { PrismaService } from '../prisma/prisma.service';
import { CreateBulletinDto, CreateDepartmentDto, CreateEmployeeDto, CreateIssueDto, CreateMeetingDto, CreateNotificationDto, CreatePositionDto, CreateProjectDto, UpdateBulletinDto, UpdateDepartmentDto, UpdateDepartmentPermissionDto, UpdateEmployeeDto, UpdateIssueDto, UpdateMeetingDto, UpdateNotificationDto, UpdatePositionDto, UpdateProjectDto } from './dto/resource.dto';

const paging = (q: PaginationDto) => ({ skip: (q.page - 1) * q.pageSize, take: q.pageSize });
@Injectable()
export class DepartmentsService {
  constructor(private prisma: PrismaService) {}
  private employeeAccess(u: AuthUser): Prisma.DepartmentWhereInput {
    return u.role === 'ADMIN' ? {} : { permissions: { some: { employee: { userId: u.id }, OR: [{ canCreate: true }, { canUpdate: true }, { canDelete: true }, { canAssignPosition: true }] } } };
  }
  async list(u: AuthUser, q: PaginationDto) { const where: Prisma.DepartmentWhereInput = { companyId: u.companyId!, ...this.employeeAccess(u), ...(q.search && { OR: [{ name: { contains: q.search, mode: 'insensitive' } }, { code: { contains: q.search, mode: 'insensitive' } }] }) }; const [data,total]=await this.prisma.$transaction([this.prisma.department.findMany({where,...paging(q),orderBy:{createdAt:'desc'}}),this.prisma.department.count({where})]); return {data,meta:pageMeta(q.page,q.pageSize,total)}; }
  async get(u: AuthUser,id:string){const row=await this.prisma.department.findFirst({where:{id,companyId:u.companyId!,...this.employeeAccess(u)}});if(!row)throw new NotFoundException('Department not found');return row;}
  ...
  async permissions(u:AuthUser,id:string){await this.get(u,id);return this.prisma.employee.findMany({where:{companyId:u.companyId!,departmentId:id},select:{id:true,fullName:true,employeeCode:true,position:true,departmentPermission:{select:{canCreate:true,canUpdate:true,canDelete:true,canAssignPosition:true}}},orderBy:{fullName:'asc'}});}
  async setPermission(u:AuthUser,id:string,employeeId:string,d:UpdateDepartmentPermissionDto){await this.get(u,id);const employee=await this.prisma.employee.findFirst({where:{id:employeeId,companyId:u.companyId!,departmentId:id}});if(!employee)throw new BadRequestException('Employee does not belong to this department');return this.prisma.departmentPermission.upsert({where:{employeeId},create:{...d,employeeId,departmentId:id,companyId:u.companyId!},update:d});}
}

@Injectable()
export class PositionsService {
  constructor(private prisma:PrismaService){}
  private async assert(u:AuthUser,departmentId:string,action?:'canCreate'|'canUpdate'|'canDelete'){if(u.role==='ADMIN')return;const permission=await this.prisma.departmentPermission.findFirst({where:{departmentId,companyId:u.companyId!,employee:{userId:u.id}}});if(!permission||!(action?permission[action]:permission.canCreate||permission.canUpdate||permission.canDelete||permission.canAssignPosition))throw new ForbiddenException('You do not have permission to manage this department');}
  async list(u:AuthUser,q:DepartmentResourcePaginationDto){if(u.role!=='ADMIN'){const employee=await this.prisma.employee.findUnique({where:{userId:u.id},select:{departmentId:true}});if(!employee?.departmentId)throw new ForbiddenException();if(q.departmentId&&q.departmentId!==employee.departmentId)throw new ForbiddenException();await this.assert(u,employee.departmentId);q.departmentId=employee.departmentId;}const where:Prisma.PositionWhereInput={companyId:u.companyId!,...(q.departmentId&&{departmentId:q.departmentId}),...(q.search&&{OR:[{name:{contains:q.search,mode:'insensitive'}},{code:{contains:q.search,mode:'insensitive'}}]})};const[data,total]=await this.prisma.$transaction([this.prisma.position.findMany({where,...paging(q),include:{department:true},orderBy:{createdAt:'desc'}}),this.prisma.position.count({where})]);return{data,meta:pageMeta(q.page,q.pageSize,total)};}
  async get(u:AuthUser,id:string){const row=await this.prisma.position.findFirst({where:{id,companyId:u.companyId!},include:{department:true}});if(!row)throw new NotFoundException('Position not found');await this.assert(u,row.departmentId);return row;}
  ...
  async create(u:AuthUser,d:CreatePositionDto){await this.assert(u,d.departmentId,'canCreate');await this.checkDepartment(u.companyId!,d.departmentId);return this.prisma.position.create({data:{...d,code:d.code.toUpperCase(),companyId:u.companyId!},include:{department:true}});}
  async update(u:AuthUser,id:string,d:UpdatePositionDto){const current=await this.get(u,id);await this.assert(u,current.departmentId,'canUpdate');if(u.role!=='ADMIN'&&d.departmentId&&d.departmentId!==current.departmentId)throw new ForbiddenException();await this.checkDepartment(u.companyId!,d.departmentId??current.departmentId);return this.prisma.position.update({where:{id},data:{...d,code:d.code?.toUpperCase()},include:{department:true}});}
  async remove(u:AuthUser,id:string){const current=await this.get(u,id);await this.assert(u,current.departmentId,'canDelete');return this.prisma.position.delete({where:{id}});}
}

@Injectable()
export class EmployeesService {
  constructor(private prisma: PrismaService) {}
  private async assert(u:AuthUser,departmentId:string,action?:'canCreate'|'canUpdate'|'canDelete'|'canAssignPosition'){if(u.role==='ADMIN')return;const permission=await this.prisma.departmentPermission.findFirst({where:{departmentId,companyId:u.companyId!,employee:{userId:u.id}}});if(!permission||!(action?permission[action]:permission.canCreate||permission.canUpdate||permission.canDelete||permission.canAssignPosition))throw new ForbiddenException('You do not have permission to manage this department');}
  async list(u:AuthUser,q:DepartmentResourcePaginationDto){if(u.role!=='ADMIN'){const employee=await this.prisma.employee.findUnique({where:{userId:u.id},select:{departmentId:true}});if(!employee?.departmentId)throw new ForbiddenException();if(q.departmentId&&q.departmentId!==employee.departmentId)throw new ForbiddenException();await this.assert(u,employee.departmentId);q.departmentId=employee.departmentId;}const where:Prisma.EmployeeWhereInput={companyId:u.companyId!,...(q.departmentId&&{departmentId:q.departmentId}),...(q.search&&{OR:[{fullName:{contains:q.search,mode:'insensitive'}},{employeeCode:{contains:q.search,mode:'insensitive'}},{email:{contains:q.search,mode:'insensitive'}}]})};const[data,total]=await this.prisma.$transaction([this.prisma.employee.findMany({where,...paging(q),include:{department:true,position:true,user:{select:{id:true,isActive:true}}},orderBy:{createdAt:'desc'}}),this.prisma.employee.count({where})]);return{data,meta:pageMeta(q.page,q.pageSize,total)};}
  async get(u:AuthUser,id:string){const row=await this.prisma.employee.findFirst({where:{id,companyId:u.companyId!},include:{department:true,position:true,user:{select:{id:true,isActive:true}}}});if(!row)throw new NotFoundException('Employee not found');if(row.departmentId)await this.assert(u,row.departmentId);else if(u.role!=='ADMIN')throw new ForbiddenException();return row;}
  ...
  async create(u:AuthUser,d:CreateEmployeeDto){if(!d.departmentId&&u.role!=='ADMIN')throw new ForbiddenException();if(d.departmentId)await this.assert(u,d.departmentId,'canCreate');if(d.positionId&&d.departmentId)await this.assert(u,d.departmentId,'canAssignPosition');await this.checkAssignment(u.companyId!,d.departmentId,d.positionId);const{password,...employee}=d;return this.prisma.$transaction(async tx=>{let userId:string|undefined;if(password){const account=await tx.user.create({data:{companyId:u.companyId!,email:d.email.toLowerCase(),passwordHash:await argon2.hash(password),role:'EMPLOYEE'}});userId=account.id;}return tx.employee.create({data:{...employee,email:d.email.toLowerCase(),employeeCode:d.employeeCode.toUpperCase(),companyId:u.companyId!,userId},include:{department:true,position:true}});});}
  async update(u:AuthUser,id:string,d:UpdateEmployeeDto){const current=await this.get(u,id);if(!current.departmentId&&u.role!=='ADMIN')throw new ForbiddenException();if(u.role!=='ADMIN'&&d.departmentId&&d.departmentId!==current.departmentId)throw new ForbiddenException();const regularChanges=Object.keys(d).some(key=>key!=='positionId'&&key!=='departmentId');if(regularChanges&&current.departmentId)await this.assert(u,current.departmentId,'canUpdate');if(d.positionId!==undefined&&d.positionId!==current.positionId&&current.departmentId)await this.assert(u,current.departmentId,'canAssignPosition');await this.checkAssignment(u.companyId!,d.departmentId??current.departmentId??undefined,d.positionId??current.positionId??undefined);const{password,...employee}=d;if(password&&!current.userId)throw new BadRequestException('Create a linked account when creating the employee');if(password&&current.userId)await this.prisma.user.update({where:{id:current.userId},data:{passwordHash:await argon2.hash(password)}});if(d.departmentId&&d.departmentId!==current.departmentId)await this.prisma.departmentPermission.deleteMany({where:{employeeId:id}});return this.prisma.employee.update({where:{id},data:{...employee,email:d.email?.toLowerCase(),employeeCode:d.employeeCode?.toUpperCase()},include:{department:true,position:true}});}
  async remove(u:AuthUser,id:string){const row=await this.get(u,id);if(row.departmentId)await this.assert(u,row.departmentId,'canDelete');return this.prisma.$transaction(async tx=>{await tx.employee.delete({where:{id}});if(row.userId)await tx.user.update({where:{id:row.userId},data:{isActive:false}});return row;});}
}
